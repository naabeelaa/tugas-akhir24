# -*- coding: utf-8 -*-
"""fixed anis imin multinomialnb.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ImDUOVwPSbZqj-gCBHcpn7xIZUr3d46O
"""

import pandas as pd
import re

df = pd.read_excel('anis imin final.xlsx')
df.head()
    ...
    def safe_join(tokens):
        if isinstance(tokens, str):
            try:
                ...
                tokens = tokens.split()
        return ' '.join([str(token) for token in tokens]) if isinstance(tokens, list) else ''

    return df[token_column].apply(safe_join)
...
def train_evaluate_nb(X_train, X_test, y_train, y_test, vectorizer_type="tfidf"):
    ...
    vectorizer = TfidfVectorizer(**params) if vectorizer_type == "tfidf" else CountVectorizer(**params)
...
def main():
    ...
    for name, results in [("TF-IDF", results_tfidf), ("BoW", results_bow)]:
        print(f"\nResults for {name}:")
...
def get_feature_importance(model, vectorizer, feature_type):
    ...
    for i, class_label in enumerate(model.classes_):
        ...
        top_indices = np.argsort(importance)[-20:]  # Get top 20 features
